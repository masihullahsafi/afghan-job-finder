...
import { improveResumeSummary, generateExperienceBulletPoints, fileToBase64, validateResume, generateCareerPath, parseResumeProfile } from '../services/geminiService';
...
export const SeekerDashboard: React.FC = () => {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => { const file = e.target.files?.[0]; if (file) { try { const base64 = await fileToBase64(file); await updateUserProfile({ avatar: `data:${file.type};base64,${base64}` }); alert("Profile picture updated!"); } catch (err) { alert("Failed to upload image."); } } };
  ...
  // UPDATED FILE UPLOAD to ensure persistence in DB for employers
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
          ...
          if (!validation.isValid) {
              alert(`Invalid Document: ${validation.reason}`);
          ...
          // CRITICAL: We save to 'documents' array AND 'resumeData' / 'resume' if it's the first resume
          // This ensures the legacy 'download resume' buttons work for employers
          const updateData: Partial<User> = { 
              documents: [newDoc, ...currentDocs]
          };

          if (currentDocs.length === 0 || confirm("Set this as your primary resume for easy applying?")) {
              updateData.resumeData = base64; // Store raw base64 for download
  ...
  const strength = 80; // Mock calculation for brevity
